...
if ($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    else
    {
        ...
        $resfeedbackfound=mysqli_query($conn,"SELECT access.feedback_code FROM regist INNER JOIN access ON regist.access_code=access.access_code WHERE regist.username='$username' AND access.feedback_code='$feedback_code' ");
        ...
        if($rowstream = $resfeedbackfound->fetch_assoc()){
            ...
            while($rowfaculty = $resfaculty->fetch_assoc()){
                ...
                $flag=0;
                while($rowsno = $ressno->fetch_assoc()){
